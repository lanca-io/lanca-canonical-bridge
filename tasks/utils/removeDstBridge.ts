...
export async function removeDstBridge(
	...
	try {
		...
		log(
			`Destination bridge successfully removed! Transaction: ${txHash}`,
			"removeDstBridge",
			chainName,
		);
	} catch (error) {
		err(`Failed to remove destination bridge: ${error}`, "removeDstBridge", chainName);
		throw error;
	}
}
